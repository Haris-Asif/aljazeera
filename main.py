...
def filter_by_date(df, label):
    ...
    cutoff = datetime.today() - timedelta(days=days_map.get(label, 0))
...
def generate_whatsapp_messages(df):
    ...
    for (sector, size), items in sorted(grouped.items()):
        ...
        section = header + body + "\n"
        if len(current_msg) + len(section) > WHATSAPP_CHAR_LIMIT:
            msg_chunks.append(current_msg.strip())
            current_msg = section
        else:
            current_msg += section
...
def main():
    ...
    if selected_name:
        ...
        nums = [clean_number(row[col].values[0]) for col in ["Contact1", "Contact2", "Contact3"] if pd.notna(row[col].values[0])]
        if nums:
        ...
        if not number or not number.strip().startswith("03"):
            ...
            if not messages:
                ...
                for idx, msg in enumerate(messages, 1):
                    ...
                    st.markdown(f"[📩 Message {idx} Link]({url})", unsafe_allow_html=True)
