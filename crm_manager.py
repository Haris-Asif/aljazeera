            ...
            for _, lead in high_priority_leads.iterrows():
                with st.expander(f"🚨 {lead['Name']} - {lead['Status']}"):
                    ...
                    if st.button("📞 Call Now", key=f"call_{lead['ID']}"):
                        ...
                        if phone_num:
                            st.markdown(f'<a href="tel:{phone_num}" style="display: inline-block; padding: 0.5rem 1rem; background-color: #25D366; color: white; text-decoration: none; border-radius: 0.5rem;">Call {phone_num}</a>', 
